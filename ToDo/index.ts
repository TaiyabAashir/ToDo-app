const subtasksContainer: HTMLElement = document.querySelector(
  ".subtasks-container"
)!;
...
`;
const addSubclassButton: HTMLElement =
  document.querySelector("#addsubtaskbutton")!;
...
filterDate.addEventListener("change", (event) => {
  const dateCreated = filterDate.value
    ? new Date(filterDate.value)
    : new Date();
  dateString =
    dateCreated.toDateString() == new Date().toDateString()
      ? "today"
...
function checkboxHandle(this: HTMLInputElement, event: MouseEvent) {
  ...
  if (this.checked) {
    (this.previousElementSibling! as HTMLElement).style.textDecoration =
      "line-through";
  } else {
    (this.previousElementSibling! as HTMLElement).style.textDecoration =
      "initial";
...
function reset() {
  ...
  ).forEach((element: HTMLInputElement) => (element.value = ""));
}
...
window.onclick = function (event) {
  ...
  if (
    eventTarget == modal ||
    (eventTarget as HTMLElement).id == "close-modal" ||
    (event.target as HTMLElement).id == "close-modal-button"
  ) {
...
document.querySelector("form")!.addEventListener("submit", (event) => {
  ...
  objStorage["subtask"] = Array.from(
    ...
    return [
      elem.value,
      (elem.parentElement!.querySelector(".subtask-time") as HTMLInputElement)
        .value,
    ];
  });
...
function showTask(objStorage: objStorage) {
  ...
  addElem.addEventListener("click", (event) =>
    editElement(event, objStorage.id!)
  );
...
function populateForm(id: string) {
  ...
  while (numOfSubtasks--) addMoreButton.click();

  let subElems: NodeListOf<HTMLInputElement> = document.querySelectorAll(
    ".create-subtask-element"
  );
  console.log(subElems);
  for (let i = 0; i < subElems.length; i++) {
    subElems[i].value = objStorage["subtask"]![i][0];
    (subElems[i].nextElementSibling as HTMLInputElement).value =
      objStorage["subtask"]![i][1];
  }
}
...
document
  ...
  .addEventListener("keyup", function (this: HTMLInputElement, event) {
    if ((event as KeyboardEvent).key == "Enter") filterBySearch(this.value);
  });
function filterBySearch(searchString: string) {
  ...
  for (let key of keyList) {
    ...
    if (key == "currentId") continue;
    let elem = JSON.parse(localStorage[key]);
    if (hasString(elem, searchString)) showTaskHighlight(elem, searchString);
  }
  currentView = "search";
}
...
function hasString(objStorage: objStorage, searchString: string) {
  ...
  if (objStorage.mt!.indexOf(searchString) != -1) return true;
...
function deleteElement(imgElem: HTMLElement, id: string) {
  if (imgElem.closest(".create-sub-task")) {
    let textContent = (
      imgElem
        .closest(".create-sub-task")!
        .querySelector(".task-text")! as HTMLElement
    ).innerText;
    let newObj = JSON.parse(localStorage[id]);
    newObj["subtask"] = newObj["subtask"].filter(
      (value: any) => value[0] != textContent
    );
...
//All task
document
  .querySelector(".button-show-all")!
  .addEventListener("click", function (this: HTMLButtonElement, event) {
    showAll(this);
  });
